...
def run_daily_job():
    ...
    # ── 并行抓取（使用 ThreadPoolExecutor 提高效率） ──────────
    print("  → 正在并行抓取各项数据…")
    with ThreadPoolExecutor(max_workers=7) as executor:
        ...
        future_rd_new_proj = executor.submit(rootdata.get_new_projects, days=1)
        future_rd_tge      = executor.submit(rootdata.get_upcoming_tge, days_ahead=7)
        future_cr_funding  = executor.submit(cryptorank.get_daily_funding, today)
        future_cr_ido      = executor.submit(cryptorank.get_upcoming_ido, days_ahead=7)
