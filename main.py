...
def run_daily_job():
    ...
    # ── 并行抓取（利用 ThreadPoolExecutor 优化性能） ─────────
    # 将原本 7 个顺序执行的网络请求改为并行执行，
    # 总耗时从所有请求之和降至最慢的一个请求的耗时（预计提速 70%-80%）。
    print(f"  → 正在并行抓取 {today_str} 的各项数据…")
    with ThreadPoolExecutor(max_workers=7) as executor:
        ...
        f_rd_new_proj = executor.submit(rootdata.get_new_projects, days=1)
        f_rd_tge      = executor.submit(rootdata.get_upcoming_tge, days_ahead=7)
        f_cr_funding  = executor.submit(cryptorank.get_daily_funding, today)
        f_cr_ido      = executor.submit(cryptorank.get_upcoming_ido, days_ahead=7)
