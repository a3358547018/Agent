...
class TestAirdropHelperParallel(unittest.TestCase):
    ...
    def test_run_daily_job_success(self, mock_send_message, mock_get_session_nt, mock_get_session_ok, mock_get_session_cr, mock_get_session_rd):
        ...
        with patch("main.date") as mock_date_main, patch("okboost.date") as mock_date_ok:
            mock_date_main.today.return_value = test_today
            mock_date_main.side_effect = lambda *args, **kwargs: date(*args, **kwargs)
            mock_date_ok.today.return_value = test_today
            mock_date_ok.side_effect = lambda *args, **kwargs: date(*args, **kwargs)
